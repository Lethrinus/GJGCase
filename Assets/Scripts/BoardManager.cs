    ...
    public class BoardManager : MonoBehaviour
    {
        ...
        private Dictionary<int, CrateBehavior> crateGrid = new Dictionary<int, CrateBehavior>();
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("InputHandler reference not set in BoardManager!");
            }
        ...
        for (int col = 0; col < boardData.columns; col++)
        {
            ...
            for (int i = validRows.Count - 1; i >= writeIndex; i--)
            {
                ...
                if (nb)
                {
                    ...
                    // Kill any active tweens on the newly spawned block.
                    nb.transform.DOKill(true);
    ...
    {
        // If using crates, build a HashSet of reserved indices from crateGrid.
        if (boardConfig.useCrates)
        {
            HashSet<int> reservedIndices = new HashSet<int>(crateGrid.Keys);
            deadlockResolver.ResolveDeadlockFullRefill(
                boardData, 
                transform, 
                boardConfig.shuffleFadeDuration, 
                boardConfig.thresholdA, 
                boardConfig.thresholdB, 
                boardConfig.thresholdC, 
                onComplete,
        ...
        {
            int startRow = boardData.rows - 3;  // bottom three rows 
        ...
        private void RemoveAdjacentCrates(List<int> group, Action onComplete)
        {
            ...
            DOVirtual.DelayedCall(totalAnimationTime, () => { onComplete?.Invoke(); });
        }
